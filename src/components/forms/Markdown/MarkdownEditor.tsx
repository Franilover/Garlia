...
function MarkdownPreviewWithSnippets({
  ...
  const blocks = React.useMemo(() => {
    ...
    const result: Array<
      { kind: "md"; text: string } | { kind: "segs"; text: string }
    > = [];
    let mdAccum = "";
    ...
    for (const line of withPlaceholders.split("\n")) {
      if (SNIPPET_LINE_RE.test(line)) {
        if (mdAccum) {
          result.push({ kind: "md", text: mdAccum });
          mdAccum = "";
        }
        const restored = line.replace(
          /\x00GATEPREVIEW_\d+\x00/g,
          (ph) => gateMap.get(ph) ?? ph,
        );
          ...
          return (
            <div
              dangerouslySetInnerHTML={{
                __html: renderMarkdown(block.text, isLibro),
              }}
              key={i}
            />
          );
...
export function MarkdownEditor({
  ...
  useEffect(() => {
    ...
    if (
      initialWidth >= SPLIT_MIN_WIDTH &&
      !modeProp &&
      !userChangedModeRef.current
    ) {
      setMode("preview");
    }
  ...
  useEffect(() => {
    ...
    if (maxHeight) {
      ...
      ta.style.height =
        (isNaN(maxPx) ? ta.scrollHeight : Math.min(ta.scrollHeight, maxPx)) +
        "px";
    } else {
  ...
  const countMatches = useCallback(
    (text: string, query: string, cs: boolean) => {
      if (!query) return 0;
      try {
        return (
          text.match(
  ...
  const findAndHighlight = useCallback(
    (dir: 1 | -1 = 1) => {
      ...
      const next =
        (findReplace.currentMatch + dir + matches.length) % matches.length;
      ta.focus();
      ta.setSelectionRange(
        matches[next],
        matches[next] + findReplace.find.length,
      );
      setFindReplace((s) => ({
        ...s,
        currentMatch: next,
  ...
  const replaceOne = useCallback(() => {
    ...
    if (
      s !== e &&
      value.slice(s, e).toLowerCase() === findReplace.find.toLowerCase()
    ) {
  ...
  useEffect(() => {
    ...
    setFindReplace((s) => ({
      ...s,
      totalMatches: total,
      currentMatch: Math.min(s.currentMatch, Math.max(0, total - 1)),
    }));
  ...
  const filteredEntities =
    wikiMenu.query.length === 0
      ? normalizedEntities
      : normalizedEntities.filter((e) =>
          e.name.toLowerCase().includes(wikiMenu.query.toLowerCase()),
        );

  const allCommands = hideBuiltinCommands
    ? extraCommands
    : [...extraCommands, ...COMMAND_ITEMS];
  const filteredItems =
    cmdMenu.query.length === 0
      ? allCommands
      : allCommands.filter(
          (item) =>
            item.keywords.some((k) =>
              k.startsWith(cmdMenu.query.toLowerCase()),
  ...
  const getCaretCoords = useCallback((ta: HTMLTextAreaElement, pos: number) => {
    ...
    const clampedTop = Math.max(
      taRect.top,
      Math.min(spanRect.top, taRect.bottom - lineHeight),
    );
    return {
      top: clampedTop + lineHeight + 2,
      left: Math.max(
        taRect.left + 8,
        Math.min(spanRect.left, taRect.right - 264),
      ),
    };
  }, []);
    ...
    (item: CommandItem) => {
      ...
      if (item.snippet) {
        ...
        requestAnimationFrame(() => {
          ...
          ta.selectionStart = ta.selectionEnd = item.cursorOffset
  ...
  const applyWikilink = useCallback(
    (entity: WikiEntity) => {
      ...
      onChange(
        value.slice(0, wikiMenu.triggerStart) + inserted + value.slice(endPos),
      );
      setWikiMenu((m) => ({ ...m, open: false }));
      requestAnimationFrame(() => {
        ta.selectionStart = ta.selectionEnd =
          wikiMenu.triggerStart + inserted.length;
        ta.focus();
      });
    },
  ...
  const commitTableEdit = useCallback(
    (newRows: string[][]) => {
      ...
      onChange(
        value.slice(0, tableEditor.tableStart) +
          newMd +
          value.slice(tableEditor.tableEnd),
      );
      setTableEditor((te) => ({
        ...te,
        rows: newRows,
        tableEnd: tableEditor.tableStart + newMd.length,
      }));
    },
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (wikiMenu.open) {
      ...
      if (filteredEntities.length > 0) {
        if (e.key === "ArrowDown") {
          ...
          setWikiMenu((m) => ({
            ...m,
            selectedIdx: (m.selectedIdx + 1) % filteredEntities.length,
          }));
          return;
        }
        if (e.key === "ArrowUp") {
          ...
          setWikiMenu((m) => ({
            ...m,
            selectedIdx:
              (m.selectedIdx - 1 + filteredEntities.length) %
    ...
    if (cmdMenu.open && filteredItems.length > 0) {
      if (e.key === "ArrowDown") {
        ...
        setCmdMenu((m) => ({
          ...m,
          selectedIdx: (m.selectedIdx + 1) % filteredItems.length,
        }));
        return;
      }
      if (e.key === "ArrowUp") {
        ...
        setCmdMenu((m) => ({
          ...m,
          selectedIdx:
            (m.selectedIdx - 1 + filteredItems.length) % filteredItems.length,
        }));
        return;
      }
      if (e.key === "Tab" || e.key === "Enter") {
    ...
    if (e.key === "Enter") {
      ...
      if (listMatch) {
        if (listMatch[2].trim() === "") {
          e.preventDefault();
          const newVal =
            value.slice(0, s - listMatch[1].length) + value.slice(s);
          onChange(newVal);
          requestAnimationFrame(() => {
    ...
    if (e.key === "[") {
      ...
      if (!hasSel && charBefore === "[" && charAfter === "]") {
        ...
        const newVal =
          currentVal.slice(0, s) + "[" + sel + "]" + currentVal.slice(e2);
        onChange(newVal);
        requestAnimationFrame(() => {
  ...
  const handleScroll = (e: React.UIEvent<HTMLElement>) => {
    ...
    if (scrollHeight > clientHeight) {
      ...
      pvRef.current.scrollTop =
        ratio * (pvRef.current.scrollHeight - pvRef.current.clientHeight);
    }
  };

  // ── Styles ────────────────────────────────────────────────────────────────
  const textareaCls =
    "flex-1 w-full h-full bg-transparent outline-none border-none resize-none font-mono placeholder:opacity-25";
    ...
    <div className={`flex flex-col w-full h-full ${className}`}>
      ...
      >
        ...
        {findReplace.open && (
          <FindReplacePanel
            ...
            onFindChange={(v) =>
              setFindReplace((s) => ({ ...s, find: v, currentMatch: 0 }))
            }
            onFindKeyDown={(e) => {
              if (e.key === "Enter") {
          ...
          {(mode === "edit" || mode === "split") && (
            ...
            >
              ...
              {sectionTitle && (
                <div
                  ...
                  style={{
                    ...
                    lineHeight: "1.625",
                    color:
                      "color-mix(in srgb, var(--color-primary,#7c6af7) 55%, transparent)",
              ...
              <textarea
                ref={(node) => {
                  (
                    taRef as React.MutableRefObject<HTMLTextAreaElement | null>
                  ).current = node;
                  if (textareaRef)
                    (
                      textareaRef as React.MutableRefObject<HTMLTextAreaElement | null>
              ...
              {cmdMenu.open && (
                <CommandMenu
                  ...
                  onHover={(idx) =>
                    setCmdMenu((m) => ({ ...m, selectedIdx: idx }))
                  }
      ...
      {tableEditor.open && tableEditor.anchorEl && (
        <TableEditorPanel
          ...
          onAddCol={() =>
            commitTableEdit(tableEditor.rows.map((r) => [...r, ""]))
          }
          onAddRow={() => {
