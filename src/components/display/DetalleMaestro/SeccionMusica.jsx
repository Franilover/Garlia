...
export const SelectorMusicaAdmin = ({ idsSeleccionados = [], onChange }) => {
  // Usa el sistema de caché/offline en lugar de un fetch manual
  const { data: todas, loading } = useSupabaseData("canciones", {
    select: "id, titulo",
    order: { campo: "titulo", asc: true },
  });
  ...
  // Marcar como montado para el Portal
          ...
          {safeIds.length > 0 ? (
            safeIds.map(id => {
              ...
              return (
                <span
                  key={id}
                  className="bg-primary text-btn-text text-[9px] font-black px-4 py-2 flex items-center gap-2 uppercase italic tracking-wider shadow-sm border border-primary/20"
            ...
            {todas.map(c => {
              ...
              return (
                <div
                  key={c.id}
                  onClick={() => toggle(c.id)}
                  className={`p-4 rounded-[var(--radius-btn)] cursor-pointer mb-1.5 flex justify-between items-center transition-all ${
                    isSelected
                      ? "bg-primary text-btn-text shadow-lg translate-x-1"
