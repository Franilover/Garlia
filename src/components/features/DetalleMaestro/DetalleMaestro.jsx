          ...
          <div className="flex flex-col lg:flex-row items-stretch border-b border-primary/5">
            <div className="w-full lg:w-[48%] bg-slate-50 p-10 lg:p-24 flex items-center justify-center relative overflow-hidden min-h-[550px] lg:min-h-[800px]">
              ...
              <div className="relative w-full aspect-square max-w-140 group">
                ...
                <motion.div layoutId="hero-img" className="relative w-full h-full rounded-full overflow-hidden border-[16px] border-white shadow-[0_40px_80px_-15px_rgba(0,0,0,0.35)] bg-white">
                  <motion.img 
                    key={imagenVisual} 
              ...
              {editMode ? (
                  ...
                  {esCriatura && (
                    <div className="pt-12 border-t border-primary/5">
                      <div className="flex items-center justify-between mb-10">
                        ...
                        <button onClick={agregarVariante} className="px-10 py-4 bg-primary text-white rounded-[1.5rem] text-[10px] font-black uppercase hover:shadow-2xl hover:-translate-y-1 transition-all flex items-center gap-4">
                      ...
                      <div className="space-y-8 max-h-[550px] overflow-y-auto pr-6 custom-scrollbar p-2">
                        {variantes.map((v, index) => (
                          <motion.div key={v.id || index} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-10 bg-slate-50 rounded-[3.5rem] border border-primary/5 relative group transition-all hover:bg-white hover:shadow-2xl">
                            <div className="flex flex-col md:flex-row gap-8 items-start">
                              <div className="w-32 h-32 rounded-3xl overflow-hidden bg-white shrink-0 shadow-lg border-4 border-white">
                                {v.imagen_url ? <img src={v.imagen_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-primary/10"><ImageIcon size={32} /></div>}
                              </div>
                              <div className="flex-1 space-y-4 w-full">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <input placeholder="Nombre Forma" value={v.tipo} onChange={(e) => { const n = [...variantes]; n[index].tipo = e.target.value; setVariantes(n); }} className="bg-white px-6 py-4 rounded-2xl text-xs font-bold text-primary border border-primary/5" />
                                  <input placeholder="URL Imagen" value={v.imagen_url} onChange={(e) => { const n = [...variantes]; n[index].imagen_url = e.target.value; setVariantes(n); }} className="bg-white px-6 py-4 rounded-2xl text-[10px] font-mono text-slate-400 border border-primary/5" />
                                </div>
                                <textarea placeholder="Descripción variante..." value={v.descripcion_variante} onChange={(e) => { const n = [...variantes]; n[index].descripcion_variante = e.target.value; setVariantes(n); }} className="w-full bg-white p-6 rounded-[2rem] text-xs italic text-slate-500 resize-none min-h-[100px] border border-primary/5" />
                              </div>
                            </div>
                            <button onClick={() => eliminarVariante(index)} className="absolute -top-4 -right-4 bg-red-500 text-white p-4 rounded-full shadow-xl opacity-0 group-hover:opacity-100 transition-all hover:scale-110"><Trash2 size={20} /></button>
                          </motion.div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="relative">
                  ...
                  <h2 className="text-6xl lg:text-8xl font-black uppercase italic text-primary leading-[0.85] tracking-tighter mb-12">
                    {varianteActiva ? <><span className="block text-3xl opacity-40 mb-2">{data.nombre}</span><span>{varianteActiva.tipo}</span></> : editNombre}
                  </h2>
                  ...
                  {esCriatura && variantes.length > 0 && (
                    <div className="flex flex-wrap gap-4 mt-16">
                      <button onClick={() => setVarianteActiva(null)} className={`px-10 py-5 rounded-[2rem] text-[12px] font-black uppercase transition-all duration-500 ${!varianteActiva ? 'bg-primary text-white shadow-2xl -translate-y-2' : 'bg-primary/5 text-primary hover:bg-primary/10'}`}>Fenotipo Base</button>
                      {variantes.map((v, i) => (
                        <button key={i} onClick={() => setVarianteActiva(v)} className={`px-10 py-5 rounded-[2rem] text-[12px] font-black uppercase transition-all duration-500 ${varianteActiva === v ? 'bg-primary text-white shadow-2xl -translate-y-2' : 'bg-primary/5 text-primary hover:bg-primary/10'}`}>{v.tipo}</button>
                      ))}
                    </div>
                  )}
                </div>
          ...
          {tieneContenidoInferior && (
            <div className="bg-slate-50 p-12 lg:p-24 grid grid-cols-1 xl:grid-cols-2 gap-24 items-start border-t border-primary/5 relative">
              ...
              <div className="space-y-10">
                ...
                {loadingRelaciones ? <div className="h-40 bg-white rounded-[3rem] w-full animate-pulse border border-primary/5" /> : (
