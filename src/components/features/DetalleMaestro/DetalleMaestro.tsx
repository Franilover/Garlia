    ...
    <div className="max-w-7xl mx-auto relative pt-10 px-4 pb-32">
      <AnimatePresence>
        {showSuccess && (
          ...
          >
            <CheckCircle2 size={24} /> "Sincronizado correctamente"
          </motion.div>
        )}
      </AnimatePresence>
            ...
            {editMode ? (
              <div className="space-y-12">
                <div className="space-y-4">
                  <label className="text-sm font-black text-slate-600 uppercase tracking-widest ml-6">Nombre</label>
                  <input 
                    value={editNombre} 
                ...
                <div className="space-y-4">
                  <label className="text-sm font-black text-slate-600 uppercase tracking-widest ml-6">Descripción</label>
                  <textarea 
                ...
                {esCriatura && (
                  <div className="space-y-6 pt-10 border-t-2 border-slate-100">
                    <div className="flex items-center justify-between">
                      <h5 className="text-sm font-black uppercase text-slate-400 tracking-widest">Variantes Existentes</h5>
                      <button onClick={agregarVariante} className="p-3 bg-primary text-white rounded-full hover:scale-110 transition-all shadow-lg"><Plus size={24}/></button>
                    </div>
                    {variantes.map((v, i) => (
                      <div key={i} className="p-8 bg-slate-50 rounded-[2rem] space-y-4 border-2 border-slate-200 shadow-sm">
                        <div className="flex gap-4">
                          <input 
                            placeholder="Nombre de la variante" 
                            className="flex-1 bg-white border-2 border-slate-200 p-4 rounded-2xl text-lg text-slate-800 font-bold outline-none focus:border-primary"
                            value={v.tipo} 
                            onChange={(e) => actualizarVariante(i, "tipo", e.target.value)}
                          />
                          <button onClick={() => eliminarVariante(i)} className="text-slate-400 hover:text-red-500 transition-colors p-2"><Trash2 size={24}/></button>
                        </div>
                        <textarea 
                          placeholder="Descripción de esta variante..." 
                          className="w-full bg-white border-2 border-slate-200 p-4 rounded-2xl text-slate-600 outline-none focus:border-primary min-h-[100px]"
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <p className="text-slate-700 text-2xl lg:text-3xl leading-[1.6] font-medium mb-12">
                  {varianteActiva ? (varianteActiva.descripcion_variante || "Sin descripción registrada.") : editNombre ? editDescripcion : "No hay datos disponibles."}
                </p>

                {/* SELECTOR DE VARIANTES EN VISTA NORMAL */}
                {esCriatura && variantes.length > 0 && (
                  <div className="mt-16">
                    <h4 className="text-xs font-black uppercase text-slate-400 tracking-[0.3em] mb-6 italic">Seleccionar Variante:</h4>
        ...
        {(esPersonaje || editMode) && (
          <div className="bg-slate-100 p-16 lg:p-24 grid grid-cols-1 xl:grid-cols-2 gap-16 border-t border-slate-200">
            <div className="space-y-8">
              <h4 className="text-sm font-black uppercase tracking-[0.4em] text-slate-800 flex items-center gap-4"><Users size={24}/> "Relaciones"</h4>
            ...
            {mostrarMusica && (
              <div className="space-y-8">
                <h4 className="text-sm font-black uppercase tracking-[0.4em] text-slate-800 flex items-center gap-4"><Music size={24}/> "Multimedia"</h4>
      ...
      {isAdmin && (
        <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[1100] flex items-center gap-4 bg-slate-900/95 backdrop-blur-xl p-5 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
          <button onClick={() => setEditMode(!editMode)} className={`px-10 py-5 rounded-full text-xs font-black uppercase tracking-[0.2em] flex items-center gap-3 transition-all ${editMode ? "bg-white text-slate-900" : "bg-primary text-white hover:scale-105"}`}>
            ...
            {editMode ? "Cerrar Editor" : "Editar Expediente"}
          </button>
