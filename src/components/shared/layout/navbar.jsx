        ...
        <AnimatePresence>
          {(openSubmenu || (userMenuOpen && user)) && (
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 15 }}
              className="absolute bottom-20 left-0 w-full bg-white border border-primary/10 rounded-4xl p-3 shadow-2xl flex flex-col gap-2 z-1001">
              
              {userMenuOpen && user && (
                <div className="flex flex-col gap-2">
                  <Link href="/personal" onClick={closeAll} className="w-full p-5 bg-primary/5 text-primary rounded-3xl font-black uppercase text-[10px] flex items-center justify-center gap-3">
                    <Sword size={18}/> Mi Personaje
                  </Link>
                  <button onClick={handleLogout} className="w-full p-4 bg-red-50 text-red-600 rounded-3xl font-black uppercase text-[10px] flex items-center justify-center gap-3">
...
const MobileSubItem = ({ href, label, icon, active, onClick }) => (
  <Link href={href} onClick={onClick} className={cn("flex flex-col items-center gap-2 p-5 rounded-3xl border transition-all", active ? "bg-primary border-primary text-white shadow-lg" : "bg-primary/5 border-transparent text-primary/40")}>
    {icon}
