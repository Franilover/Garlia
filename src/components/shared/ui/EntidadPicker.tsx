...
import { X, Search, Loader2, Sword, Package, Check, User } from "lucide-react";
  ...
  useEffect(() => {
    ...
    fetch(`/api/entidades?tipo=${tipoFijo ?? ""}`)
      ...
      .then(d => {
        ...
        const criaturas = (d.criaturas ?? []).map((x: any) => ({
          id: x.id, nombre: x.nombre, tipo: "criatura" as const,
          subtipo: x.habitat, imagen_url: x.imagen_url, descripcion: x.descripcion,
        }));

        const personajes = (d.personajes ?? []).map((x: any) => ({
          id: x.id, nombre: x.nombre, tipo: "personaje" as const,
          subtipo: x.ocupacion, imagen_url: x.imagen_url, descripcion: x.descripcion,
        }));
              ...
              <div className="w-1/2 border-r border-primary/8 flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto py-1">
                  {loading ? (
                    ...
                    <button key={e.id} onClick={() => setSelected(e)} className={cn("w-full flex items-center gap-3 px-4 py-2.5 text-left transition-all", selected?.id === e.id ? "bg-primary/10" : "hover:bg-primary/5")}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[9px] text-primary/35 uppercase tracking-widest font-black">{e.subtipo ?? e.tipo}</p>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="w-1/2 flex flex-col overflow-y-auto">
                {selected ? (
                  <div className="p-5 flex flex-col gap-5 flex-1">
                    ...
                    <input value={palabra} onChange={e => setPalabra(e.target.value)} placeholder="Palabra en el texto..." className="w-full px-3 py-2 rounded-xl border border-primary/12 text-sm focus:outline-none" />
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center opacity-20">{getIcon(tab, 24)}<p className="text-[10px] font-black uppercase mt-2">Selecciona un {tab}</p></div>
                )}
              </div>
            </div>

            <div className="px-6 py-4 border-t border-primary/8 shrink-0 flex items-center justify-between">
              <button onClick={onClose} className="text-[10px] font-black uppercase text-primary/40">Cancelar</button>
              <button onClick={handleInsert} disabled={!selected || !palabra.trim()} className="px-5 py-2 rounded-xl text-[10px] font-black uppercase bg-primary text-white disabled:opacity-30">Insertar Drop</button>
            </div>
          </motion.div>
        </>
      )}
