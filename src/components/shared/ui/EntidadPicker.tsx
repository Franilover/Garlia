    ...
    fetch(`/api/entidades?tipo=${tipoFijo ?? ""}`)
      ...
      .then(d => {
        ...
        const personajes = (d.data?.personajes ?? []).map((x: any) => ({
          ...
          subtipo: x.visible ? "Poblador" : "Misterioso",
        }));
                  ...
                  {loading ? (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={cn("text-[11px] font-bold truncate", selected?.id === e.id ? "text-primary" : "text-primary-dark")}>
                          {e.nombre}
                        </p>
                        <p className="text-[9px] text-primary/35 uppercase tracking-widest font-black">
                          {e.subtipo}
                        </p>
                      </div>
                    </button>
