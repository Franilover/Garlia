...
import {
  Plus, Trash2, X, Edit3, Save, User, List, Music,
  EyeOff, AlertCircle, ChevronDown, Link2, ExternalLink,
...
const textareaCls = "w-full bg-bg-main border border-primary/5 rounded-[var(--radius-card)] p-6 text-primary text-sm italic font-serif leading-relaxed outline-none focus:bg-white-custom focus:border-primary/30 transition-all resize-none";

// ─── Reducers ─────────────────────────────────────────────────────────────────
const initialModalState = {
  showAddModal: false, showEditSecModal: false, showLinksModal: false,
  showFullLyricsModal: false, showMassEditModal: false,
  selectedSec: null, linkEditandoIndex: null, procesando: false,
};
...
const initialFormState = {
  nuevoNombre: "", nuevaLetraEs: "", nuevaLetraEn: "", nuevaLetraJp: "", nuevaLetraRomaji: "",
  editSecNombre: "", editSecEs: "", editSecEn: "", editSecJp: "", editSecRomaji: "",
  nuevoLinkTitulo: "", nuevoLinkUrl: "",
};
...
const LinkSection = ({ links, isAdmin, onOpenModal, onEdit, onDelete }) => (
  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
    ...
    <div className="flex items-center justify-between mb-4">
      ...
      {isAdmin && (
        <BtnIcon variant="ghost" size="sm" onClick={onOpenModal} className="border-none w-6 h-6">
          <Plus size={14} />
        </BtnIcon>
      )}
    </div>
        ...
        {links.map((link, i) => (
          <div key={i} className="flex items-center justify-between group">
            <a href={link.url} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 text-primary hover:text-[var(--primary)] transition-colors text-xs font-bold truncate flex-1">
              <ExternalLink size={10} className="flex-shrink-0" />
              <span className="truncate">{link.titulo}</span>
            </a>
            {isAdmin && (
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <BtnIcon variant="ghost" size="sm" onClick={() => onEdit(i)} className="border-none w-6 h-6 text-primary/40 hover:text-primary"><Edit3 size={10} /></BtnIcon>
                <BtnIcon variant="danger" size="sm" onClick={() => onDelete(i)} className="border-none w-6 h-6"><Trash2 size={10} /></BtnIcon>
              </div>
            )}
          </div>
        ))}
...
const FullLyricsModal = ({ isOpen, onClose, secciones, idiomaActivo }) => {
  ...
  const handleCopy = () => {
    ...
    const texto = secciones.map(s => {
      const letra = s[`letra_${lang}`];
      return letra ? `${s.nombre_seccion}\n\n${letra}` : "";
    }).filter(Boolean).join("\n\n---\n\n");
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1">
                    <button onClick={() => setZoom(Math.max(0.5, zoom - 0.1))} className="w-6 h-6 flex items-center justify-center bg-primary/5 rounded text-primary hover:bg-primary/10 transition-colors text-sm font-bold">-</button>
                    <span className="text-[9px] font-black text-primary/50 min-w-[38px] text-center">{Math.round(zoom * 100)}%</span>
                    <button onClick={() => setZoom(Math.min(2, zoom + 0.1))} className="w-6 h-6 flex items-center justify-center bg-primary/5 rounded text-primary hover:bg-primary/10 transition-colors text-sm font-bold">+</button>
                  </div>
                  <Btn variant="ghost" size="sm" icon={<Copy size={12} />} onClick={handleCopy}>Copiar</Btn>
                  <BtnIcon variant="ghost" size="sm" onClick={onClose} className="border-none text-primary/40 hover:text-red-500"><X size={18} /></BtnIcon>
                </div>
              </div>
            </div>
            <div className="flex-1 overflow-auto bg-bg-main selection:bg-primary/10">
              <div className="w-full h-fit p-8 md:p-20 transition-all duration-300 ease-out origin-top"
    ...
    {isOpen && (
      <div className="fixed inset-0 z-[130] flex items-center justify-center p-6">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
          className="bg-white-custom w-full max-w-md rounded-[var(--radius-card)] p-10 shadow-2xl relative z-10 border border-primary/10">
          <BtnIcon variant="ghost" onClick={onClose} className="absolute top-8 right-8 border-none text-primary/20 hover:text-primary"><X size={20} /></BtnIcon>
          ...
          <form onSubmit={onSave} className="space-y-4 mb-8">
            <input type="text" placeholder="TÍTULO" value={titulo} onChange={e => onTituloChange(e.target.value)}
              className="w-full bg-bg-main border-b border-primary/10 py-3 text-sm font-bold text-primary outline-none focus:border-primary uppercase" />
            <input type="url" placeholder="URL" value={url} onChange={e => onUrlChange(e.target.value)}
              className="w-full bg-bg-main border-b border-primary/10 py-3 text-sm font-medium text-primary outline-none focus:border-primary" />
            <div className="flex gap-2">
              <Btn type="submit" loading={isProcessing} disabled={!titulo.trim() || !url.trim()} className="flex-1">
                {isEditing ? "Guardar" : "Añadir"}
              </Btn>
              {isEditing && <Btn type="button" variant="ghost" onClick={onClose}>Cancelar</Btn>}
            </div>
          </form>
            ...
            {links?.map((link, i) => (
              <motion.div key={i} layout className="flex items-center justify-between p-3 rounded-[var(--radius-btn)] border bg-primary/5 border-primary/10 hover:bg-primary/10 transition-colors">
                ...
                <div className="flex gap-1">
                  <BtnIcon variant="ghost" size="sm" onClick={() => onEdit(i)} className="border-none text-primary/40 hover:text-primary w-7 h-7"><Edit3 size={14} /></BtnIcon>
                  <BtnIcon variant="danger" size="sm" onClick={() => onDelete(i)} className="border-none w-7 h-7"><Trash2 size={14} /></BtnIcon>
                </div>
              </motion.div>
            ))}
          </div>
...
const SeccionModal = ({ isOpen, isEditing, onClose, isProcessing, nombre, onNombreChange, es, onEsChange, en, onEnChange, jp, onJpChange, romaji, onRomajiChange, onSave, onDelete = null }) => {
  ...
  const tabContent = { es: [es, onEsChange, "Escribe la letra en español..."], en: [en, onEnChange, "Escribe la letra en inglés..."], jp: [jp, onJpChange, "日本語で歌詞を書く..."], romaji: [romaji, onRomajiChange, "Kakikomi romaji..."] };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
            className="bg-white-custom w-full max-w-3xl rounded-[var(--radius-card)] p-10 shadow-2xl relative z-10 border border-primary/10 max-h-[90vh] overflow-y-auto">
            <BtnIcon variant="ghost" onClick={onClose} className="absolute top-8 right-8 border-none text-primary/20 hover:text-primary z-10"><X size={20} /></BtnIcon>
            ...
            <div className="space-y-6">
              <input type="text" placeholder="NOMBRE DE LA SECCIÓN (Ej: ESTROFA, CORO)" value={nombre}
                onChange={e => onNombreChange(e.target.value.toUpperCase())}
                className="w-full bg-bg-main border-b-2 border-primary/10 py-3 text-sm font-black text-primary outline-none focus:border-primary uppercase tracking-widest" />
              <div className="flex gap-1 bg-primary/5 p-1 rounded-[var(--radius-btn)] border border-primary/10">
                {IDIOMAS.map(lang => (
                  <button key={lang.id} type="button" onClick={() => setActiveTab(lang.id)}
                    className={`flex-1 py-2 rounded-[var(--radius-input)] font-black text-[9px] uppercase transition-all ${activeTab === lang.id ? "bg-primary shadow-md" : "text-primary/40 hover:text-primary"}`}
              ...
              <div className="min-h-[200px]">
                {Object.entries(tabContent).map(([lang, [val, onChange, placeholder]]) =>
                  activeTab === lang ? (
                    <textarea key={lang} value={val as string} onChange={e => (onChange as any)(e.target.value)}
                      rows={8} className={textareaCls} placeholder={placeholder as string} />
                  ) : null
                )}
              </div>
              <div className="flex gap-4 pt-4">
                <Btn onClick={onSave} loading={isProcessing} disabled={!nombre.trim()} icon={<Save size={14} />} className="flex-1">
                  Guardar Sección
                </Btn>
                {isEditing && onDelete && (
                  <Btn variant="danger" onClick={onDelete} loading={isProcessing} icon={<Trash2 size={14} />} className="px-6">
                    Eliminar
                  </Btn>
                )}
              </div>
...
export default function CancionDetallesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const [{ data: { session } }, { data: cancionData, error: errorC }, { data: seccionesData, error: errorS }] = await Promise.all([
    ...
    } catch (err) { console.error(err); setErrorAcceso(true); }
  ...
  const handleSaveLink = async (e) => {
    ...
    } catch (error) { alert("Error: " + (error.message || "No se pudo guardar el enlace")); }
    finally { dispatchModal({ type: "SET_PROCESANDO", payload: false }); }
  };
  ...
  const removeLink = async (index) => {
    ...
    try {
      ...
      dispatchModal({ type: "SET_EDITING_LINK", payload: null });
    } catch (error) { alert("Error: " + (error.message || "No se pudo eliminar")); }
  };
  ...
  const handleUpdateEstado = async (nuevoEstado) => {
    ...
    } catch (error) { alert("Error: " + (error.message || "No se pudo actualizar")); }
  };
  ...
  const handleCrearSeccion = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("secciones_cancion").insert([{
        cancion_id: id, nombre_seccion: nuevoNombre.trim(), orden: maxOrden + 1,
        letra_es: nuevaLetraEs.trim() || null, letra_en: nuevaLetraEn.trim() || null,
        letra_jp: nuevaLetraJp.trim() || null, letra_romaji: nuevaLetraRomaji.trim() || null,
      }]).select().single();
      if (error) throw error;
      setSecciones(prev => [...prev, data]);
      dispatchForm({ type: "RESET_NUEVA" });
      dispatchModal({ type: "CLOSE_ADD" });
    } catch (error) { alert("Error: " + (error.message || "No se pudo crear")); }
    finally { dispatchModal({ type: "SET_PROCESANDO", payload: false }); }
  };
  ...
  const handleUpdateSeccion = async () => {
    ...
    dispatchModal({ type: "SET_PROCESANDO", payload: true });
    try {
      const updates = { nombre_seccion: editSecNombre.trim(), letra_es: editSecEs.trim() || null, letra_en: editSecEn.trim() || null, letra_jp: editSecJp.trim() || null, letra_romaji: editSecRomaji.trim() || null };
  ...
  const handleMassUpdate = async (seccionesEditadas) => {
    try {
      ...
      for (const sec of secciones.filter(s => !idsEditadas.has(String(s.id)))) {
        await supabase.from("secciones_cancion").delete().eq("id", sec.id);
      }
      const seccionesInsertadas = [];
      for (const sec of seccionesNuevas) {
        const { data, error } = await supabase.from("secciones_cancion").insert([{ cancion_id: id, nombre_seccion: sec.nombre_seccion, letra_es: sec.letra_es || "", letra_en: sec.letra_en || "", letra_jp: sec.letra_jp || "", letra_romaji: sec.letra_romaji || "", orden: sec.orden }]).select().single();
        if (error) throw error;
        seccionesInsertadas.push({ tempId: sec.id, real: data });
      }
      for (const sec of seccionesExistentes) {
        const { error } = await supabase.from("secciones_cancion").update({ nombre_seccion: sec.nombre_seccion, letra_es: sec.letra_es || "", letra_en: sec.letra_en || "", letra_jp: sec.letra_jp || "", letra_romaji: sec.letra_romaji || "", orden: sec.orden }).eq("id", sec.id);
        if (error) throw error;
      }
      setSecciones(seccionesEditadas.map(sec => {
    ...
    } catch (error) { alert("Error al guardar: " + (error.message || "Error desconocido")); throw error; }
  };
  ...
  return (
    <div className="min-h-screen bg-bg-main pb-20 relative">
      <LinksModal
        ...
        titulo={formState.nuevoLinkTitulo} onTituloChange={val => dispatchForm({ type: "SET_LINK", payload: { nuevoLinkTitulo: val } })}
        url={formState.nuevoLinkUrl}       onUrlChange={val => dispatchForm({ type: "SET_LINK", payload: { nuevoLinkUrl: val } })}
        onSave={handleSaveLink} links={cancion?.links || []} onEdit={prepararEdicionLink} onDelete={removeLink}
        isEditing={modalState.linkEditandoIndex !== null}
      />
      <SeccionModal
        isOpen={modalState.showAddModal} isEditing={false}
        onClose={() => { dispatchModal({ type: "CLOSE_ADD" }); dispatchForm({ type: "RESET_NUEVA" }); }}
        isProcessing={modalState.procesando}
        nombre={formState.nuevoNombre}         onNombreChange={val => dispatchForm({ type: "SET_NUEVA_SECCION", payload: { nuevoNombre: val } })}
        es={formState.nuevaLetraEs}            onEsChange={val => dispatchForm({ type: "SET_NUEVA_SECCION", payload: { nuevaLetraEs: val } })}
        en={formState.nuevaLetraEn}            onEnChange={val => dispatchForm({ type: "SET_NUEVA_SECCION", payload: { nuevaLetraEn: val } })}
        jp={formState.nuevaLetraJp}            onJpChange={val => dispatchForm({ type: "SET_NUEVA_SECCION", payload: { nuevaLetraJp: val } })}
        romaji={formState.nuevaLetraRomaji}    onRomajiChange={val => dispatchForm({ type: "SET_NUEVA_SECCION", payload: { nuevaLetraRomaji: val } })}
        onSave={handleCrearSeccion}
      />
      <SeccionModal
        isOpen={modalState.showEditSecModal} isEditing={true}
        onClose={() => { dispatchModal({ type: "CLOSE_EDIT_SEC" }); dispatchForm({ type: "RESET_EDIT" }); }}
        isProcessing={modalState.procesando}
        nombre={formState.editSecNombre}       onNombreChange={val => dispatchForm({ type: "SET_EDIT_SECCION", payload: { editSecNombre: val } })}
        es={formState.editSecEs}               onEsChange={val => dispatchForm({ type: "SET_EDIT_SECCION", payload: { editSecEs: val } })}
        en={formState.editSecEn}               onEnChange={val => dispatchForm({ type: "SET_EDIT_SECCION", payload: { editSecEn: val } })}
        jp={formState.editSecJp}               onJpChange={val => dispatchForm({ type: "SET_EDIT_SECCION", payload: { editSecJp: val } })}
        romaji={formState.editSecRomaji}       onRomajiChange={val => dispatchForm({ type: "SET_EDIT_SECCION", payload: { editSecRomaji: val } })}
      ...
      <MassEditModal isOpen={modalState.showMassEditModal} onClose={() => dispatchModal({ type: "CLOSE_MASS_EDIT" })} secciones={secciones} isProcessing={modalState.procesando} onSave={handleMassUpdate} cancionId={id} />
      ...
      <div className={`mx-auto px-6 grid md:grid-cols-[280px_1fr] gap-16 mt-4 transition-all duration-500 ${idiomasActivos.length > 1 ? "max-w-7xl" : "max-w-5xl"}`}>
        <aside className="space-y-6">
          ...
          {isAdmin && !cancion?.visible && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-primary text-btn-text rounded-[var(--radius-btn)] flex items-center justify-center gap-3 shadow-xl">
          ...
          <LinkSection links={cancion?.links} isAdmin={isAdmin}
            onOpenModal={() => { dispatchForm({ type: "RESET_LINK" }); dispatchModal({ type: "OPEN_LINKS" }); }}
            onEdit={prepararEdicionLink} onDelete={removeLink} />
        </aside>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between mb-8 border-b border-primary/10 pb-4">
              <div className="flex items-center gap-4">
                ...
                {secciones.length > 0 && (
                  <>
                    <Btn variant="ghost" size="sm" icon={<FileText size={12} />} onClick={() => dispatchModal({ type: "OPEN_FULL_LYRICS" })}>Lectura</Btn>
                    {isAdmin && <Btn variant="ghost" size="sm" icon={<Layers size={12} />} onClick={() => dispatchModal({ type: "OPEN_MASS_EDIT" })}>Editor Maestro</Btn>}
                  </>
                )}
              </div>
              {isAdmin && (
                <BtnIcon onClick={() => { dispatchForm({ type: "RESET_NUEVA" }); dispatchModal({ type: "OPEN_ADD" }); }}>
                  <Plus size={18} />
                </BtnIcon>
              )}
            </div>
                ...
                <motion.div key={seccion.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="relative group">
                  <div className={`bg-white-custom border border-primary/5 rounded-[var(--radius-card)] transition-all hover:border-primary/20 hover:shadow-2xl hover:shadow-primary/5 ${idiomasActivos.length > 1 ? "p-8 md:p-12" : "p-10"}`}>
                    <div className="flex items-center justify-between mb-8">
                      ...
                      {isAdmin && (
                        <BtnIcon variant="ghost" size="sm" onClick={() => openEditSec(seccion)}
                          className="border-none opacity-0 group-hover:opacity-100 hover:bg-primary hover:text-btn-text">
                          <Edit3 size={14} />
                        </BtnIcon>
                      )}
                    </div>
            ...
            {secciones.length === 0 && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                ...
                {isAdmin && (
                  <Btn onClick={() => { dispatchForm({ type: "RESET_NUEVA" }); dispatchModal({ type: "OPEN_ADD" }); }} className="rounded-full">
