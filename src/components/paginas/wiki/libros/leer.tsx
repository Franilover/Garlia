...
// ─── Hook: desbloquear personajes al completar un capítulo ───────────────────
function useDesbloquearPersonajes(capId: string, personajesIds: string[] | undefined) {
  ...
  const disparar = useCallback(async () => {
    ...
    for (const personajeId of personajesIds) {
      // Insertar solo si no existe ya (upsert seguro por constraint)
      const { error } = await supabase
        .from("descubrimientos_personajes")
        .insert({ perfil_id: perfilId, personaje_id: personajeId })
        .select()
        .single();
      // error code 23505 = unique_violation (ya existía) → no contar como nuevo
...
function PersonajesDesbloqueadosToast({
  personajesIds,
  onClose,
}: {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="px-5 py-3 flex items-center justify-between border-b border-primary/8"
...
// ─── Separador final de capítulo con trigger de desbloqueo ────────────────────
function FinCapituloSeparador({
  cap,
  onVisible,
}: {
...
function CapituloScrollBlock({
  ...
  const { disparar, mostrarCelebration, desbloqueados, cerrar } = useDesbloquearPersonajes(cap.id, cap.personajes_ids);
  ...
  return (
    ...
    >
      ...
      {/* Separador con IntersectionObserver — dispara desbloqueo al llegar */}
