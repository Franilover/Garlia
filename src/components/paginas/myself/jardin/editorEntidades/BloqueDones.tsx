...
function useAsignados(personajeId: string) {
  ...
  const remove = async (id: string) => {
    await supabase.from("personaje_dones").delete()
      .eq("personaje_id", personajeId).eq("don_id", id);
    ...
    <div className="space-y-2">
      <div className="relative" ref={ref}>
        <input
          ...
          disabled={noEspecie}
          placeholder={noEspecie ? "Asigná una especie primero…" : "Buscar dones compatibles…"}
        ...
        {open && disponibles.length === 0 && (
          <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white-custom border border-primary/15 rounded-xl shadow-xl px-3 py-3">
            <p className="text-[9px] text-primary/25 text-center italic">Todos los dones compatibles ya están asignados</p>
          </div>
        )}
