...
import { RelojDigital } from "@/features/ensayos/components/calendario/relojDigital";
import ArmarioPage from "@/features/ensayos/components/ropa/ropa";
import type { ModoCalendario } from "@/features/ensayos/components/calendario/types";
import { VistaMes } from "@/features/ensayos/components/calendario/vistaMes";
import { VistaSemanal } from "@/features/ensayos/components/calendario/vistaSemanal";
import { PaginaEjercicios } from "@/features/ensayos/views/ejercicios/ejerciciosComponent";
import { PaginaHobbys } from "@/features/ensayos/views/hobbys/hobbysComponent";
...
import { PaginaPendientes } from "@/features/ensayos/views/pendientes/pendientesComponent";
...
interface HomeDashboardProps {
  ...
  onUpdateEvento?: (
    id: string,
    datos: { titulo?: string; tipo?: string; fecha?: string },
  ) => Promise<void>;
  onDeleteEvento?: (id: string) => Promise<void>;
  onToggleEstado?: (
    libroId: string,
    estado: "leyendo" | "leido" | "pendiente",
    add: boolean,
  ) => void;
  onCrearLibro?: () => void;
}
...
export function HomeDashboard({
  ...
  const totalPalabras = useMemo(
    () =>
      ensayos.reduce(
        (acc, e) =>
          acc + (e.contenido?.split(/\s+/).filter(Boolean).length || 0),
        0,
      ),
    [ensayos],
  );
  ...
  const pendientes = useMemo(
    () => tareas.filter((t) => !t.completada).slice(0, 10),
    [tareas],
  );
  ...
  return (
    ...
    >
      ...
      `}</style>
      <div
        className="hd-outer-padding"
        style={{ maxWidth: 1400, margin: "0 auto", padding: "40px 40px 80px" }}
      >
        {/* ── Stats mínimo arriba ── */}
        <p
          className="hd-stats"
        ...
        >
          {ensayos.length} notas · {totalPalabras.toLocaleString("es-ES")}{" "}
          palabras
        ...
        >
          <div style={{ background: "var(--bg-main)", padding: "26px 24px" }}>
            ...
            >
              ...
              >
                {tagActivo ? "Notas" : "Todas las notas"} ·{" "}
              ...
              {notasFiltradas.map((e, i) => (
                ...
                >
                  <button
                    ...
                    onMouseLeave={(el) =>
                      (el.currentTarget.style.background = "var(--bg-main)")
                    }
                  >
                    <p
                      style={{
