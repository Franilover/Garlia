  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-1 text-[9px] font-black text-primary/35">
        <span className="text-primary/25">P</span>
        {proteinas.toFixed(0)}g
      </div>
      <div className="flex items-center gap-1 text-[9px] font-black text-primary/35">
        <span className="text-primary/25">C</span>
        {carbos.toFixed(0)}g
      </div>
      <div className="flex items-center gap-1 text-[9px] font-black text-primary/35">
        <span className="text-primary/25">G</span>
        {grasas.toFixed(0)}g
      </div>
    </div>
  );
  ...
  const items = [
    {
      ...
      value: kcal,
      unit: "",
      icon: <Flame className="text-accent" size={10} />,
    },
    {
      label: "Prot",
      value: proteinas,
      unit: "g",
      icon: <Dumbbell className="text-blue-400" size={10} />,
    },
    {
      label: "Carb",
      value: carbos,
      unit: "g",
      icon: <Wheat className="text-amber-400" size={10} />,
    },
    {
      label: "Gras",
      value: grasas,
      unit: "g",
      icon: <Droplets className="text-emerald-400" size={10} />,
    },
  ];
  return (
    <div className="grid grid-cols-4 gap-2">
      {items.map((m) => (
        <div
          key={m.label}
          className="bg-bg-main border border-primary/8 rounded-(--radius-btn) py-2 text-center"
        >
          ...
          <p className="text-[11px] font-black text-primary">
            {m.value.toFixed(0)}
            <span className="text-[8px] text-primary/25">{m.unit}</span>
          </p>
        </div>
      ))}
    </div>
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-primary/8 shrink-0">
          ...
          <BtnIcon
            className="border-none bg-primary/8 text-primary/40"
            variant="ghost"
            onClick={onClose}
          >
            <X size={16} />
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          ...
          <div>
            ...
            {receta.descripcion && (
              <p className="text-[11px] text-primary/45 mt-1.5 leading-relaxed">
                {receta.descripcion}
              </p>
            )}
            <div className="flex items-center gap-4 mt-3 pt-3 border-t border-primary/8">
              ...
              <div className="flex items-center gap-1.5 text-[10px] font-bold text-primary/45">
                <ChefHat className="text-primary/25" size={12} />{" "}
                {receta.dificultad}
              </div>
            </div>
          </div>
              ...
              <ol className="space-y-3">
                {instruccionesList.map((paso, i) => (
                  <li key={i} className="flex gap-3">
                    ...
                    <p className="text-[11px] font-bold leading-relaxed text-primary/65 uppercase">
                      {paso}
                    </p>
                  </li>
                ))}
                          ...
                          onChange={(e) =>
                            setPendingIng((p) =>
                              p
                                ? {
                                    ...p,
                                    qty: Math.max(0.1, Number(e.target.value)),
                                  }
                                : p,
                            )
                          }
              ...
              {formData.ingredientes.length > 0 && (
                <div className="space-y-1.5">
                  {formData.ingredientes.map((ing, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 bg-bg-main border border-primary/8 rounded-(--radius-btn) px-3 py-2"
                    >
                      <div className="flex-1 min-w-0">
                        <span className="text-[11px] font-black uppercase text-primary">
                          {ing.nombre}
                        </span>
                        <span className="text-[9px] text-primary/30 ml-2">
                          {ing.cantidad}
                        </span>
                      </div>
                      <MacroBar
              ...
              {formData.instrucciones.length > 0 && (
                <div className="space-y-1.5">
                  {formData.instrucciones.map((paso, idx) => (
                    <div
                      key={idx}
                      className="flex items-start gap-3 bg-bg-main border border-primary/8 rounded-(--radius-btn) px-4 py-2.5"
                    >
                      <span className="w-5 h-5 rounded-full bg-bg-menu text-menu-text text-[8px] font-black flex items-center justify-center shrink-0 mt-0.5">
                        {idx + 1}
                      </span>
                      <span className="text-[11px] font-bold text-primary/65 uppercase flex-1 leading-relaxed">
                        {paso}
                      </span>
                      <button
      ...
      <header className="sticky top-0 z-10 bg-bg-main/90 backdrop-blur-xl border-b border-primary/10">
        ...
        <div className="max-w-7xl mx-auto px-5 pb-3 flex items-center gap-2 flex-wrap">
          ...
          >
            Todas
            <span
              className={`text-[8px] px-1.5 py-0.5 rounded-full font-black ${catFilter === null ? "bg-btn-text/20" : "bg-primary/5 text-primary/35"}`}
            >
              {recipes.length}
            </span>
          </button>
          ...
          {CATEGORIAS.map(({ label, emoji }) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[8px] px-1 py-0.5 rounded-full font-black ${active ? "bg-btn-text/20" : "bg-primary/5 text-primary/35"}`}
                >
                  {count}
                </span>
              </button>
              ...
              {filteredRecipes.length === 0 && !loading ? (
                ...
                filteredRecipes.map((receta, i) => (
                  <RecipeCard
                    ...
                    index={i}
                    receta={receta}
                    onSelect={() => setSelectedReceta(receta)}
                  />
                ))
              )}
            </AnimatePresence>
