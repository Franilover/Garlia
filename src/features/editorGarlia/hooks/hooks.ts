...
// ─── Helpers Dexie ────────────────────────────────────────────────────────────
// dexieRead era una copia local idéntica a dexieReadAll de dexieHelpers.ts
// (mismo cuerpo, solo distinto nombre). Se usa el módulo central bajo alias
...
export function usePersonajesDeEspecie(
  especieNombre: string | null | undefined,
) {
  const [personajes, setPersonajes] = useState<Personaje[]>([]);
  const [loading, setLoading] = useState(false);
