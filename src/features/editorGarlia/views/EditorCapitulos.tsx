      ...
      <AnimatePresence>
        {previewOpen && (
          <div className="fixed inset-0 z-[200] flex flex-col">
            <MotionDiv
              animate={{ opacity: 1 }} className="absolute inset-0 bg-bg-main" exit={{ opacity: 0 }}
              initial={{ opacity: 0 }}
            />
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-center justify-between px-6 py-3 bg-white-custom/80 backdrop-blur-md border-b border-primary/10 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button className="p-1.5 rounded-lg hover:bg-primary/8 text-primary/30 hover:text-primary transition-all" onClick={() => setPreviewOpen(false)}>
                    <X size={16}/>
                  </button>
                </div>
              </div>
        ...
        <div className="shrink-0 px-4 sm:px-8 pt-4 sm:pt-6 pb-3 sm:pb-4 border-b border-primary/8 space-y-3">
          ...
          <div className="flex items-center">
            <button
              ...
              onClick={onVolver}
            >
              <ChevronRight className="rotate-180 transition-transform group-hover:-translate-x-0.5" size={10} />
              Biblioteca
            </button>
          </div>
          ...
          <div className="flex items-start gap-2">
            {editingTitle ? (
              <div className="flex-1 flex items-center gap-2">
                ...
                />
                <button className="p-2 rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-all disabled:opacity-40" disabled={savingMeta} onClick={handleSaveTitle}>
                  {savingMeta ? <Loader2 className="animate-spin" size={14}/> : <Check size={14}/>}
                </button>
                <button className="p-2 rounded-lg hover:bg-primary/5 text-primary/30 hover:text-primary transition-all" onClick={() => { setEditingTitle(false); setTitulo(cap.titulo_capitulo); }}>
                  <X size={14}/>
                </button>
              </div>
            ) : (
              <div className="flex items-start gap-2 flex-1 min-w-0">
                ...
                <button className="shrink-0 p-1.5 rounded-lg hover:bg-primary/8 text-primary/25 hover:text-primary transition-all mt-0.5" onClick={() => setEditingTitle(true)}>
                  <Pencil size={12}/>
                </button>
              </div>
            )}
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <button className="p-2 rounded-lg hover:bg-primary/8 text-primary/30 hover:text-primary transition-all disabled:opacity-30" disabled={saveStatus === "saving"}
                title="Guardar (Ctrl+S)" onClick={() => doSave(contenido)}>
                <Save size={14}/>
              </button>
              <button className="p-2 rounded-lg hover:bg-red-500/10 text-primary/20 hover:text-red-400 transition-all" title="Eliminar capítulo" onClick={handleDelete}>
                <Trash2 size={13}/>
              </button>
              <button
              ...
              {capVisibilidad === "programado" && (
                editingFecha ? (
                  <span className="flex items-center gap-1.5">
                    <Calendar size={9}/>
                    <input autoFocus className="bg-primary/5 border border-primary/20 rounded-lg px-2 py-0.5 text-[9px] font-bold text-primary outline-none focus:border-primary/40 transition-colors" type="date" value={fecha}
                    ...
                    />
                    <button className="p-1 rounded bg-primary/10 text-primary hover:bg-primary/20 transition-all disabled:opacity-40" disabled={savingMeta} onClick={handleSaveFecha}>
                      {savingMeta ? <Loader2 className="animate-spin" size={10}/> : <Check size={10}/>}
                    </button>
                    <button className="p-1 rounded hover:bg-primary/5 text-primary/30 hover:text-primary transition-all" onClick={() => { setEditingFecha(false); setFecha(toDateInput(cap.fecha_publicacion)); }}>
                      <X size={10}/>
                    </button>
                  </span>
                ) : (
                  <button className="flex items-center gap-1 hover:text-primary transition-colors group/fecha" title="Editar fecha" onClick={() => setEditingFecha(true)}>
                    <Calendar size={9}/>
                    ...
                    <Pencil className="opacity-0 group-hover/fecha:opacity-60 transition-opacity ml-0.5" size={8}/>
                  </button>
                )
              )}
      ...
      {focusMode && (
        <div className="shrink-0 flex items-center justify-between px-3 sm:px-8 py-2 sm:py-3 border-b border-primary/5">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <EstadisticasEscritura compact={true} texto={contenido}/>
            <SaveIndicator status={saveStatus}/>
            <button className="p-1.5 rounded-lg hover:bg-primary/8 text-primary/25 hover:text-primary transition-all" onClick={onToggleFocus}>
              <Minimize2 size={13}/>
            </button>
          </div>
        </div>
  ...
  return (
    <ModalBase onClose={onClose}>
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <CampoInput autoFocus label="Título" placeholder="TÍTULO DEL LIBRO…" value={titulo} onChange={setTitulo} />
      ...
      <form className="space-y-4 max-h-[70vh] overflow-y-auto pr-1" onSubmit={handleSubmit}>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {ESTADOS.map(est => (
              <button key={est} className={`px-3 py-1.5 rounded-[var(--radius-btn)] text-[9px] font-black uppercase tracking-wide border transition-all ${
                  estado === est
        ...
        <div className="pt-2">
          <BotonSubmit
            disabled={!titulo.trim()}
            labelLoading={<><Loader2 className="animate-spin" size={13}/>Guardando…</>}
...
const ModalEditarCapitulo = ({
  ...
  return (
    <ModalBase onClose={onClose}>
      ...
      <form className="space-y-4 max-h-[72vh] overflow-y-auto pr-1" onSubmit={handleSubmit}>
        <CampoInput autoFocus label="Título" placeholder="NOMBRE DEL CAPÍTULO…" value={titulo} onChange={setTitulo} />
        <CampoInput label="Orden" placeholder="1" type="number" value={orden} onChange={setOrden} />
        <SelectorVisibilidad
...
export function EditorCapitulosPanel() {
  ...
  return (
    <>
      ...
      {showNuevoLibro && <ModalNuevoLibro onClose={() => setShowNuevoLibro(false)} onCreated={handleLibroCreado} />}
      {showNuevoCap && selectedLibroId && <ModalNuevoCapitulo libroId={selectedLibroId} ordenSiguiente={capitulos.length + 1} onClose={() => setShowNuevoCap(false)} onCreated={handleCapCreada} />}
      {editandoLibro && <ModalEditarLibro libro={editandoLibro} onClose={() => setEditandoLibro(null)} onSaved={handleLibroEditado} />}
      {editandoCap && <ModalEditarCapitulo cap={editandoCap} onClose={() => setEditandoCap(null)} onSaved={handleCapEditada} />}
    </>
  );
}
