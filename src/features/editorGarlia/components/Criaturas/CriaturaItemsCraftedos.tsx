...
export function useCraftedItems(criaturaId: string) {
  ...
  const load = useCallback(async () => {
    ...
    const remoteItems: CraftedItem[] = (data ?? []).map((r: any) => ({
      ...
      itemImg:
        (Array.isArray(r.items) ? r.items[0]?.imagen_url : r.items?.imagen_url) ?? null,
    }));
  ...
  const add = async (item: ItemMin) => {
    ...
    setItems((prev) => [
      ...prev,
      { crafterId: tempId, itemId: item.id, itemName: item.nombre, itemImg: item.imagen_url ?? null },
    ]);
    ...
    if (!error && data) {
      setItems((prev) =>
        prev.map((i) => (i.crafterId === tempId ? { ...i, crafterId: data.id } : i)),
      );
      try {
        if (db)
      ...
      ch.postMessage({ itemId: item.id, origen: "Artificial", sub_origen: null });
      ch.close();
    } else {
