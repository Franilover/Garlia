 ...
 * ──────────────────────
 * Hook `useHechizosDelPersonaje` + componente `PersonajeHechizos`.
 * Muestra los hechizos disponibles (filtrados por grupos de la especie)
 * y permite togglearlos para el personaje dado.
 *
 * Solo se monta cuando `especieEsMagica` es true (ver PersonajeSidebarPanel).
 *
 * Ruta destino:
 *   src/features/editorGarlia/components/PersonajeHechizos.tsx
 */
...
export function useHechizosDelPersonaje(
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      // 1. Hechizos disponibles filtrados por grupos de la especie
      ...
      // 2. Hechizos ya seleccionados para este personaje
      let selIds: string[] = [];
      try {
        if (db) {
          const local: any[] =
