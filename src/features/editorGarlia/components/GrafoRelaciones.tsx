...
function esFamilia(tipo: string): boolean {
  return TIPOS_FAMILIA.has(tipo.toLowerCase().trim());
}

// ─── Carga de datos ───────────────────────────────────────────────────────────

async function cargarDatosGrafo(personajeId: string): Promise<DatosGrafo> {
  // 1. Cargar datos del personaje centro
...
function GrafoD3({
  ...
  useEffect(() => {
    ...
    // Separar nodos familia vs resto
    const familiaNodos = datos.enlaces
      .filter(e => esFamilia(e.tipo))
      .map(e => e.target);
    ...
    // Familia: distribuir verticalmente
    const enlacesFam = datos.enlaces.filter(e => esFamilia(e.tipo));
    const enlacesLib = datos.enlaces.filter(e => !esFamilia(e.tipo));

    // Nodos familia arriba y abajo
    const tiposFamArriba = ["padre", "madre", "abuelo", "abuela", "ancestro", "tío", "tía"];
    const famArriba = enlacesFam.filter(e => tiposFamArriba.includes(e.tipo.toLowerCase())).map(e => e.target);
    const famAbajo  = enlacesFam.filter(e => !tiposFamArriba.includes(e.tipo.toLowerCase())).map(e => e.target);

    famArriba.forEach((id, i) => {
      const total = famArriba.length;
      const x = cx + (i - (total - 1) / 2) * 110;
      posiciones[id] = { x, y: cy - 130, fixed: true };
    });
    famAbajo.forEach((id, i) => {
      const total = famAbajo.length;
      const x = cx + (i - (total - 1) / 2) * 110;
      posiciones[id] = { x, y: cy + 130, fixed: true };
    });

    // Nodos libres: alrededor del centro en círculo lateral
    const libresIds = enlacesLib.map(e => e.target).filter(id => !familiaNodos.includes(id));
    const radioLibre = Math.min(W, H) * 0.36;
    libresIds.forEach((id, i) => {
      const total = libresIds.length;
      // Ángulo: evitar zona vertical (top/bottom) — distribuir en los lados
    ...
    // Nodos sin posición aún (edge case)
    datos.nodos.forEach(n => {
      if (!posiciones[n.id]) {
        posiciones[n.id] = { x: cx + Math.random() * 80 - 40, y: cy + Math.random() * 80 - 40, fixed: false };
      }
    });

    // Construir nodes/links para D3
    ...
    const sim = d3.forceSimulation(nodes)
      .force("link", d3.forceLink(links).id((d: any) => d.id).distance(d =>
        esFamilia((d as any).tipo) ? 135 : 150
      ).strength(0.8))
      .force("charge", d3.forceManyBody().strength(-220))
      .force("collision", d3.forceCollide(50))
      .force("center", d3.forceCenter(cx, cy).strength(0.04));

    // Fijar nodos con posición fija
    nodes.forEach(n => {
      if (posiciones[n.id]?.fixed) { n.fx = posiciones[n.id].x; n.fy = posiciones[n.id].y; }
    });
    ...
    // Gradiente línea familia
    const gradFam = defs.append("linearGradient").attr("id", "grad-fam").attr("gradientUnits", "userSpaceOnUse");
    gradFam.append("stop").attr("offset", "0%").attr("stop-color", primary).attr("stop-opacity", 0.5);
    gradFam.append("stop").attr("offset", "100%").attr("stop-color", primary).attr("stop-opacity", 0.2);

    // Gradiente línea libre
    const gradLib = defs.append("linearGradient").attr("id", "grad-lib").attr("gradientUnits", "userSpaceOnUse");
    gradLib.append("stop").attr("offset", "0%").attr("stop-color", accent || primary).attr("stop-opacity", 0.4);
    gradLib.append("stop").attr("offset", "100%").attr("stop-color", accent || primary).attr("stop-opacity", 0.15);
    ...
    // Etiqueta del tipo sobre la línea
    ...
    nodoEl.each(function(d: any) {
      ...
      if (d.img_url) {
        ...
        const s = r * 0.55;
        el.append("circle")
          .attr("r", s * 0.45).attr("cy", -s * 0.2)
          .attr("fill", primary).attr("opacity", 0.2);
        el.append("path")
          .attr("d", `M${-s * 0.7},${s * 0.6} Q${-s * 0.7},${s * 0.05} 0,${s * 0.05} Q${s * 0.7},${s * 0.05} ${s * 0.7},${s * 0.6}`)
          .attr("fill", primary).attr("opacity", 0.18);
      }
    });
    ...
    // Punto centro destacado
    nodeG.selectAll("g.nodo")
...
export function GrafoRelaciones({
  ...
  // Medir el contenedor para que el SVG ocupe todo
  useEffect(() => {
          ...
          >
            ...
            <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-primary/[0.06] bg-primary/[0.03]">
              <div className="flex items-center gap-2">
                ...
                {personajeNombre && (
                  <span className="text-[9px] font-black text-primary/30">— {personajeNombre}</span>
                )}
              </div>
              <button
            ...
            <div className="shrink-0 flex items-center gap-4 px-4 py-1.5 border-b border-primary/[0.04]">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[7px] font-black uppercase tracking-widest text-primary/35">Familia</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-5 h-px border-t border-dashed border-primary/30" />
                <span className="text-[7px] font-black uppercase tracking-widest text-primary/35">Otros</span>
              </div>
