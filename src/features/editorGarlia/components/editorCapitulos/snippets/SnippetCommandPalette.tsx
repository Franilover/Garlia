...
const CATS: { id: SnippetType; label: string; icon: string; color: string; keywords: string[] }[] = [
  { id: "drop",    label: "Drop",    icon: "⚔",  color: "#a09af0", keywords: ["drop", "entidad", "personaje", "criatura", "item"] },
  { id: "choice",  label: "Choice",  icon: "🔀", color: "#5aabf5", keywords: ["choice", "decisión", "boton", "botón"] },
  { id: "use",     label: "Use",     icon: "👆", color: "#f07574", keywords: ["use", "usar", "ítem", "inventario"] },
  { id: "gate",    label: "Gate",    icon: "🚪", color: "#e09a2a", keywords: ["gate", "puerta", "condicional"] },
  { id: "section", label: "Sección", icon: "›",  color: "#8b83e8", keywords: ["section", "sección", "seccion", "ancla"] },
  { id: "imagen",  label: "Imagen",  icon: "🖼",  color: "#2dc896", keywords: ["imagen", "img", "foto", "dibujo"] },
  { id: "sound",   label: "Sonido",  icon: "♪",  color: "#e87aaa", keywords: ["sonido", "sound", "música", "musica", "audio"] },
];
...
const S = {
  popover: {
    ...
    borderRadius: 14,
    boxShadow: "0 20px 60px rgba(0,0,0,.7), 0 2px 8px rgba(0,0,0,.4)",
  ...
  row: (active: boolean): React.CSSProperties => ({
    ...
    background: active ? "color-mix(in srgb, var(--color-primary, #7c6af7) 10%, transparent)" : "transparent",
  ...
  label: { fontSize: 12, fontWeight: 700, color: "var(--foreground, #fff)", lineHeight: 1.2 } as React.CSSProperties,
  sublabel: {
  ...
  kbd: {
    ...
    border: "1px solid color-mix(in srgb, var(--foreground, #fff) 10%, transparent)",
    borderRadius: 4, padding: "1px 5px", marginLeft: "auto", flexShrink: 0,
  } as React.CSSProperties,
  empty: {
    padding: "20px 12px", fontSize: 11, textAlign: "center" as const,
    color: "color-mix(in srgb, var(--foreground, #fff) 25%, transparent)",
  },
  insertBtn: (color: string): React.CSSProperties => ({
    ...
    fontSize: 10, fontWeight: 900, textTransform: "uppercase" as const, letterSpacing: ".1em",
    color, cursor: "pointer", transition: "background .12s",
    display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
  }),
  backBtn: {
    background: "none", border: "none", cursor: "pointer",
    color: "color-mix(in srgb, var(--foreground, #fff) 40%, transparent)",
    fontSize: 18, lineHeight: 1, padding: "0 2px 0 0",
  ...
  selectedPill: (color: string): React.CSSProperties => ({
...
function flattenTree(nodes: any[]): { name: string; url: string; path: string }[] {
  const result: { name: string; url: string; path: string }[] = [];
  function walk(ns: any[], prefix: string) {
    for (const n of ns) {
      if (n.url) result.push({ name: n.name, url: n.url, path: prefix ? `${prefix} / ${n.name}` : n.name });
...
function FormDrop({ initialRaw, onInsert, onBack, query }: {
  initialRaw?: string; onInsert: (s: string) => void; onBack: () => void; query?: string;
}) {
  const init = parseSnippetRaw(initialRaw);
  const initialId = init?.kind === "drop" ? (init as any).entidadId : undefined;

  type Ent = { id: string; nombre: string; tipo: "personaje" | "criatura" | "item"; subtipo?: string; imagen_url?: string };
  const TIPO_CFG = {
  ...
  useEffect(() => {
    fetch("/api/entidades")
      ...
      .then(d => {
        if (!d.ok) return;
        const list: Ent[] = [
          ...(d.data?.personajes ?? []).map((x: any) => ({ id: x.id, nombre: x.nombre, tipo: "personaje" as const, subtipo: x.ocupacion, imagen_url: x.img_url || x.imagen_url })),
          ...(d.data?.criaturas  ?? []).map((x: any) => ({ id: x.id, nombre: x.nombre, tipo: "criatura"  as const, subtipo: x.habitat,   imagen_url: x.img_url || x.imagen_url })),
          ...(d.data?.items      ?? []).map((x: any) => ({ id: x.id, nombre: x.nombre, tipo: "item"      as const, subtipo: x.categoria,  imagen_url: x.imagen_url })),
  ...
  const filtered = useMemo(() =>
    q ? all.filter(e => e.nombre.toLowerCase().includes(q.toLowerCase()) || e.tipo.includes(q.toLowerCase())) : all,
    [all, q]
  );
          ...
          return (
            <div key={e.id} style={S.row(i === active || e.id === initialId)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ ...S.sublabel, color: cfg.color }}>{e.tipo}{e.subtipo ? ` · ${e.subtipo}` : ""}</div>
              </div>
...
function FormChoice({ initialRaw, listaSecciones = [], onInsert, onBack }: {
  initialRaw?: string; listaSecciones?: { id: string; label: string }[];
  onInsert: (s: string) => void; onBack: () => void;
}) {
  const init = parseSnippetRaw(initialRaw);
  const [texto,  setTexto]  = useState(init?.kind === "choice" ? (init as any).texto  : "");
  const [target, setTarget] = useState(init?.kind === "choice" ? (init as any).target : "");
  const inputRef = useRef<HTMLInputElement>(null);
  useEffect(() => { inputRef.current?.focus(); }, []);
  const snippet = texto.trim() && target.trim() ? `[[choice|${texto.trim()}|${target.trim()}]]` : "";
              ...
              {listaSecciones.map(sec => (
                <div key={sec.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ ...S.sublabel, fontFamily: "var(--font-mono, monospace)" }}>{sec.id}</div>
                  </div>
...
function FormSection({ initialRaw, onInsert, onBack }: { initialRaw?: string; onInsert: (s: string) => void; onBack: () => void }) {
  const init = parseSnippetRaw(initialRaw);
  const [label, setLabel] = useState(init?.kind === "section" ? (init as any).label : "");
  const [id, setId]       = useState(init?.kind === "section" ? (init as any).id    : "");
  const inputRef = useRef<HTMLInputElement>(null);
  useEffect(() => { inputRef.current?.focus(); }, []);
...
function FormUse({ initialRaw, listaSecciones = [], onInsert, onBack }: {
  initialRaw?: string; listaSecciones?: { id: string; label: string }[];
  onInsert: (s: string) => void; onBack: () => void;
}) {
  const init = parseSnippetRaw(initialRaw);
  const [palabra,    setPalabra]    = useState(init?.kind === "use" ? (init as any).label : "");
  ...
  useEffect(() => {
    fetch("/api/entidades?tipo=item")
      .then(r => r.json())
      .then(d => { if (d.ok) setItems((d.data?.items ?? []).map((x: any) => ({ id: x.id, nombre: x.nombre }))); })
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => { inputRef.current?.focus(); }, []);

  const filtered = useMemo(() => items.filter(i => i.nombre.toLowerCase().includes(q.toLowerCase())), [items, q]);
  const snippet = palabra.trim() && item && targetOk
        ...
        {!item ? (
          <div>
            ...
            <div style={{ maxHeight: 120, overflowY: "auto", marginTop: 4 }}>
              {loading && <div style={S.empty}>Cargando…</div>}
              {filtered.map((it, i) => (
                <div key={it.id} style={S.row(i === active)} onClick={() => setItem(it)} onMouseEnter={() => setActive(i)}>
...
function FormGate({ initialRaw, onInsert, onBack }: { initialRaw?: string; onInsert: (s: string) => void; onBack: () => void }) {
  const [item, setItem]            = useState<{ id: string; nombre: string } | null>(null);
  ...
  const filtered = useMemo(() => items.filter(i => i.nombre.toLowerCase().includes(q.toLowerCase())), [items, q]);
  const snippet = item && tieneTexto.trim()
...
function FormImagen({ initialRaw, onInsert, onBack, query }: {
  initialRaw?: string; onInsert: (s: string) => void; onBack: () => void; query?: string;
}) {
  const init = parseSnippetRaw(initialRaw);
  ...
  useEffect(() => {
    fetch("/api/dibujos").then(r => r.json())
      .then(d => { if (d.ok) setAll(flattenTree(d.tree)); })
      .finally(() => setLoading(false));
  }, []);
  ...
  const filtered = useMemo(() =>
    q ? all.filter(f => f.name.toLowerCase().includes(q.toLowerCase()) || f.path.toLowerCase().includes(q.toLowerCase())) : all,
    [all, q]
  );
...
function FormSound({ onInsert, onBack, query }: {
  onInsert: (s: string) => void; onBack: () => void; query?: string;
}) {
  const [all, setAll]           = useState<{ name: string; url: string; path: string }[]>([]);
  const [loading, setLoading]   = useState(true);
  const [q, setQ]               = useState(query ?? "");
  ...
  useEffect(() => {
    fetch("/api/sonidos").then(r => r.json())
      .then(d => { if (d.ok) setAll(flattenTree(d.tree)); })
      .finally(() => setLoading(false));
  }, []);
  ...
  const togglePlay = (url: string) => {
    ...
    if (selected === url && playing) { audioRef.current.pause(); setPlaying(false); }
    else { audioRef.current.src = url; audioRef.current.volume = volume; audioRef.current.play(); setPlaying(true); setSelected(url); }
  };
    ...
    <>
      ...
      <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8, maxHeight: 380, overflowY: "auto" }}>
        ...
        <div style={{ maxHeight: 180, overflowY: "auto" }}>
          ...
          {!loading && filtered.length === 0 && <div style={S.empty}>Sin resultados</div>}
          {filtered.map((f, i) => (
            <div key={f.url} style={{ ...S.row(i === active || selected === f.url),
...
export function SnippetCommandPalette({
  ...
  useEffect(() => {
    const fn = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (selectedType) { setSelectedType(null); setQ(""); setChildQuery(""); }
        else onClose();
      }
    };
    ...
    for (const cat of CATS) {
      for (const kw of cat.keywords) {
        if (lower === kw || lower.startsWith(kw + " ")) {
          return { matchedCat: cat.id as SnippetType, searchQ: lower.slice(kw.length).trim() };
        }
      }
    }
  ...
  const filteredCats = useMemo(() => {
    ...
    return CATS.filter(c =>
      c.keywords.some(k => k.includes(searchQ)) || c.label.toLowerCase().includes(searchQ)
    );
  }, [matchedCat, searchQ]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") { e.preventDefault(); setActiveIdx(v => Math.min(v+1, filteredCats.length-1)); }
  ...
  const handleInsert = useCallback((raw: string) => { onInsert(raw); onClose(); }, [onInsert, onClose]);
  const handleBack   = useCallback(() => { setSelectedType(null); setQ(""); setChildQuery(""); }, []);
    ...
    <div ref={containerRef} style={{ ...S.popover, top: pos.top, left: pos.left }}>
      ...
      {!selectedType && (
        <>
          ...
          <div style={S.list}>
            {filteredCats.length === 0 && <div style={S.empty}>Sin resultados</div>}
            {filteredCats.map((cat, i) => (
              <div key={cat.id} style={S.row(i === activeIdx)}
                onClick={() => { setChildQuery(searchQ); setSelectedType(cat.id); }}
                onMouseEnter={() => setActiveIdx(i)}>
                <span style={S.iconBox(cat.color)}>{cat.icon}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={S.label}>{cat.label}</div>
                  {searchQ && <div style={{ ...S.sublabel, color: cat.color }}>buscar «{searchQ}»</div>}
                </div>
          ...
          <div style={{ padding: "6px 12px 8px", borderTop: "1px solid color-mix(in srgb,var(--foreground) 5%,transparent)" }}>
            <span style={{ ...S.sublabel, fontSize: 8, opacity: 0.7 }}>
              tipo + nombre · ej: <em style={{ color: "color-mix(in srgb,var(--color-primary,#7c6af7) 70%,transparent)", fontStyle: "normal" }}>imagen castillo</em>
            </span>
          </div>
        </>
      )}
