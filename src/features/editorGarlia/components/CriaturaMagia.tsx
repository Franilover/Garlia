 ...
 * CriaturaMagia.tsx
 * ──────────────────
 * Hooks `useHechizoCriatura` + `useDonCriatura` + componentes
 * `CriaturaMagiaUI`, `CriaturaHechizos`, `CriaturaDones` y `CriaturaMagia`.
 *
 * Solo se monta cuando `grupoEsMagico` es true para hechizos.
 * Dones se muestran siempre.
 *
 * Ruta destino:
 *   src/features/editorGarlia/components/CriaturaMagia.tsx
 */
...
// ─── Helpers: leer/sincronizar relaciones en Dexie ───────────────────────────
...
// ─── Helper: compatibilidad de grupo ─────────────────────────────────────────
...
// ─── Helper: detecta si algún grupo es "mágico" ───────────────────────────────
  ...
  const load = useCallback(async () => {
    ...
    if (navigator.onLine) {
      const { data } = await supabase
        .from("personaje_hechizos").select("hechizo_id").eq("personaje_id", criaturaId);
...
function useDonCriatura(criaturaId: string) {
  ...
  const load = useCallback(async () => {
    ...
    if (navigator.onLine) {
      const { data } = await supabase
        .from("personaje_dones").select("don_id").eq("personaje_id", criaturaId);
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        >
          ...
          <X
            className={`transition-transform duration-150 ${open ? "rotate-45" : "rotate-0"}`}
            size={9}
      ...
      {asignados.length === 0 && !open ? (
        <div className="flex items-center gap-2 px-3 py-2 opacity-35">
          ...
          <p className="text-[8px] font-black uppercase tracking-widest" style={{ color: "color-mix(in srgb, var(--primary) 30%, transparent)" }}>
            Sin {label.toLowerCase()}
          </p>
        </div>
      ) : (
        asignados.map((e) => (
          <div key={e.id} className="group flex items-center gap-2 px-3 py-1.5 transition-all hover:bg-primary/5">
            <span
...
// ─── Wrappers para separar los hooks (no se puede condicionar hooks) ──────────
function CriaturaHechizosInner({ criaturaId, gruposActuales }: { criaturaId: string; gruposActuales: string[] }) {
  const { catalogo, ids, loading, add, remove } = useHechizoCriatura(criaturaId);
  return <CriaturaMagiaUI add={add} catalogo={catalogo} gruposActuales={gruposActuales} icon={Sparkles} ids={ids} label="Hechizos" loading={loading} remove={remove} />;
}

function CriaturaDonesInner({ criaturaId, gruposActuales }: { criaturaId: string; gruposActuales: string[] }) {
  const { catalogo, ids, loading, add, remove } = useDonCriatura(criaturaId);
  return <CriaturaMagiaUI add={add} catalogo={catalogo} gruposActuales={gruposActuales} icon={Star} ids={ids} label="Dones" loading={loading} remove={remove} />;
}

// ─── CriaturaMagia — componente principal ─────────────────────────────────────
export function CriaturaMagia({
  criaturaId,
  gruposActuales,
  mostrarHechizos,
}: {
  criaturaId: string;
  /** IDs de los grupos de la criatura (para filtrar compatibilidad). */
  gruposActuales: string[];
  /** true si algún grupo de la criatura es "mágico". */
  mostrarHechizos: boolean;
}) {
  return (
    <>
      {mostrarHechizos && (
        <CriaturaHechizosInner criaturaId={criaturaId} gruposActuales={gruposActuales} />
      )}
      <CriaturaDonesInner criaturaId={criaturaId} gruposActuales={gruposActuales} />
    </>
  );
}
