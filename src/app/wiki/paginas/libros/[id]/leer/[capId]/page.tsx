...
// Tipo extendido para incluir fecha_publicacion en la lista
...
export default function Lector() {
    ...
    const fetchDatos = async () => {
      ...
      try {
        ...
        if (queryRes.error || !queryRes.data) {
          ...
          setListaCapitulos(queryRes.data.listaCapitulos); // Asegúrate que incluye fecha_publicacion
  ...
  // --- NAVEGACIÓN CON FILTRO DE FECHA ---
  const hoy = new Date().toISOString().split('T')[0];
  const indiceActual = listaCapitulos.findIndex(c => c.id === capId);

  // Busca el anterior más cercano que ya esté publicado (o cualquiera si es admin)
  const anteriorCap = listaCapitulos
    .slice(0, indiceActual)
    .reverse()
    .find(c => isAdmin || c.fecha_publicacion <= hoy);

  // Busca el siguiente más cercano que ya esté publicado (o cualquiera si es admin)
  const siguienteCap = listaCapitulos
          ...
          <AnimatePresence>
            {editMode ? (
              ...
              >
                ...
                >
                  <Save size={24} />
                  <span className="font-black text-[10px] uppercase pr-1">
                    {saving ? "..." : "Guardar"}
                  </span>
                </button>
              </motion.div>
          ...
          <footer className="mt-20 pt-10 border-t border-[#6B5E70]/10 flex flex-col items-center gap-8">
            ...
            <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
              ...
              <button
                onClick={() =>
                  anteriorCap &&
                  router.push(`/wiki/paginas/libros/${id}/leer/${anteriorCap.id}`)
                }
              ...
              {/* BOTÓN SIGUIENTE — se muestra solo si la fecha ya llegó */}
              <button
                onClick={() =>
                  siguienteCap
            ...
            {/* MENSAJE SI EL SIGUIENTE AÚN NO SE PUBLICÓ (solo para no-admins) */}
            {!isAdmin && !siguienteCap && listaCapitulos[indiceActual + 1] && (
              <p className="text-[#6B5E70]/40 font-bold text-[10px] uppercase tracking-widest italic text-center">
                Próximo capítulo el{" "}
                {new Date(listaCapitulos[indiceActual + 1].fecha_publicacion).toLocaleDateString(
                  "es-ES",
