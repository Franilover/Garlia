      ...
      <div className="max-w-5xl w-full">
        ...
        >
          ...
          <motion.p
            ...
            className="text-primary/60 font-medium tracking-wide text-sm md:text-base"
          >
            Mi espacio creativo
          </motion.p>
        </motion.div>
...
const MenuCard = ({ href, title, description, icon, delay, hasNewContent, onClick }: any) => (
  ...
  >
    <Link href={href} className="group block relative" onClick={onClick}>
      {/*
        ✅ bg-white/80 → bg-bg-menu/60  (usa variable CSS, funciona en claro y oscuro)
        ✅ border-primary/10 → border-primary/20 (más visible en oscuro)
        ✅ group-hover:bg-white → group-hover:bg-bg-menu (no fuerza blanco en oscuro)
      */}
      <div className="bg-bg-menu/60 backdrop-blur-sm border-2 border-primary/20 rounded-2xl md:rounded-3xl p-4 md:p-8 h-full transition-all duration-500 group-hover:border-primary group-hover:bg-bg-menu group-hover:shadow-[0_25px_60px_rgba(0,0,0,0.12)] group-hover:-translate-y-2 group-hover:scale-[1.02]">
        ...
        <div className="space-y-1 md:space-y-2 mb-4">
          {/* ✅ text-primary en vez de color fijo — se adapta al modo oscuro */}
          ...
          <p className="text-xs md:text-sm font-medium text-primary/70 group-hover:text-primary transition-colors">
            {description}
          </p>
        </div>

        <div className="absolute bottom-3 right-4 md:bottom-6 md:right-8 text-primary/[0.04] font-black text-4xl md:text-7xl select-none group-hover:text-primary/10 transition-all">
          {title[0]}
        </div>
      </div>
    </Link>
